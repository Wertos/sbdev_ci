...
class Torrent extends MY_Controller
{
    ...
    public function index($id)
    {
        ...
        $bookmarks = $this->details_model->check_bookmarks(
            $id,
            $this->data["curuser"]->id
        );
        ...
        $comments =
            $details->comments > 0
                ? $this->details_model->get_comments($id, $limit, $offset)
                : [
                    "rows" => "",
                    "num_rows" => "",
                ];
        ...
        $moderate =
            $this->data["logged_in"] && $this->data["admin_mod"] ? true : false;
        ###comments end
        $data = [
            "details" => $details,
            ...
            "commentform" => $this->data["logged_in"]
                ? $this->load->view(
                    "templates/" .
                        $this->config->item("default_theme") .
                        "/helpers/commentform",
            ...
            "edit" =>
                $this->data["logged_in"] &&
                ($this->data["curuser"]->id == $details->owner || $moderate)
                    ? anchor(
                        "torrent/edit/" . $details->id,
            ...
            "delete" => $moderate
                ...
                : "",
            "related" => $this->load->view(
                "templates/" .
                    $this->config->item("default_theme") .
                    "/helpers/related",
                [
                    "rows" => $this->details_model->get_related(
                        $title,
                        $id,
                        $details->category
                    ),
    ...
    public function edit($id)
    {
        ...
        $this->form_validation->set_rules(
            "poster",
            "Постер",
            "trim|required|prep_url"
        );
        $this->form_validation->set_rules(
            "descr",
            "Описание",
            "trim|required|min_length[50]"
        );
        $this->form_validation->set_rules(
            "category",
            "Категория",
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data["can_comment"] = form_checkbox(
                "can_comment",
                "accept",
                $details->can_comment === "yes" ? true : false
            );
            ...
            $this->session->set_flashdata(
                "info",
                "Торрент был успешно отредактирован!"
            );
    ...
    public function add()
    {
        if (!$this->data["logged_in"]) {
            $this->session->set_flashdata(
                "info",
                "Доступ к данной странице только для зарегистрированных пользователей!"
            );
        ...
        $this->form_validation->set_rules(
            "poster",
            "Постер",
            "trim|required|prep_url|valid_img"
        );
        $this->form_validation->set_rules(
            "descr",
            "Описание",
        ...
        if ($this->form_validation->run() && $this->upload->do_upload("file")) {
            ...
            if (!isset($dict)) {
                $torrent_errors .=
                    "<p>Что за хрень ты загружаешь? Это не бинарно-кодированый файл!</p>";
            }
            //save file to server?
            ...
            [$dname, $pieces] = [$info["name"], $info["pieces"]];
            ...
            if (isset($totallen)) {
                ...
                foreach ($flist as $fn) {
                    ...
                    if ($ffe == "Thumbs.db") {
                        $torrent_errors .=
                            "<p>В торрентах запрещено держать файлы Thumbs.db!</p>";
                    }
                }
                $type = "multi";
            }
            ...
            if ($q) {
                $torrent_errors .=
                    "<p>Данний релиз уже есть на сайте: " .
                    anchor("torrent/" . $q->id . "-" . $q->url, $q->name) .
                    "</p>";
            }
        } else {
        ...
        if ($this->form_validation->run() == false || $torrent_errors != "") {
            ...
            $array = [
                ...
                "url" => title_url($this->input->post("title", true)),
                "added" => time(),
                "info_hash" => function_exists("hex2bin")
                    ? hex2bin($infohash)
                    : pack("H*", $infohash),
                "size" => $totallen,
            ...
            if (empty($dict["announce-list"]) && !empty($dict["announce"])) {
                $dict["announce-list"][] = [$dict["announce"]];
            }
    ...
    public function delete($id)
    {
        ...
        if (
            file_exists(
                $this->config->item("public_folder") .
                    "upload/torrents/" .
                    $details->file_name .
                    ".torrent"
            )
    ...
    public function download($id)
    {
        ...
        $details = $this->details_model->get_details($id);
        $torrent_file =
            "public/upload/torrents/" . $details->file_name . ".torrent";
        if (file_exists($torrent_file) && $details) {
            ...
            if ((int) $details->category === $this->config->item("adult_cat")) {
                $torrent->announce(
                    $this->config->item("announce_urls")["adult"]
                );
                $torrent->comment(base_url() . "torrent/" . $id);
                $torrent->save(
                    $this->config->item("public_folder") . "/temp/" . $name
                );
            } else {
                $torrent->announce(
                    $this->config->item("announce_urls")["open"]
                );
                $torrent->comment(base_url() . "torrent/" . $id);
                $torrent->save(
                    $this->config->item("public_folder") . "/temp/" . $name
    ...
    public function bookmarks($id = null)
    {
        if ($this->data["curuser"] !== false) {
            if ($this->input->is_ajax_request()) {
                ...
                $data = $this->details_model->bookmarks(
                    $id,
                    $this->data["curuser"]->id
                );
                ...
                $data["class"] == "book"
                    ? $this->session->set_flashdata(
                        "info",
                        "Закладка на раздачу успешно добавлена"
                    )
                    : $this->session->set_flashdata(
                        "info",
                        "Закладка на раздачу успешно удалена"
                    );
                redirect("torrent/" . $id, "refresh");
            } elseif ($id === null and !$this->input->is_ajax_request()) {
