...
class Torrent extends MY_Controller
{
    ...
    public function index($id)
    {
        ...
        $bookmarks = $this
            ->details_model
            ->check_bookmarks($id, $this->data['curuser']
            ->id);
        $this
            ->details_model
            ->update_view($id);
        ...
        $this
            ->breadcrumb
            ->append($details->catname, $details->caturl);
        $this
            ->breadcrumb
            ->append($details->name);
        ...
        $comments = ($details->comments > 0 ? $this
            ->details_model
            ->get_comments($id, $limit, $offset) : array(
            'rows' => '',
            'num_rows' => ''
        ));
        ...
        $data = array(
            ...
            'comments' => $this
                ...
                ->view('templates/' . $this
                ->config
                ->item('default_theme') . '/helpers/comments', array(
                'comments' => $comments['rows']
            ) , true) ,
            'pagination' => $this
                ->pagination
                ->create_links() ,
            'commentform' => ($this->data['logged_in'] ? $this
                ->load
                ->view('templates/' . $this
                ->config
                ->item('default_theme') . '/helpers/commentform', array(
                'id' => $details->id
            ) , true) : '') ,
            'category' => anchor($details->caturl, $details->catname) ,
            'edit' => ($this->data['logged_in'] && ($this->data['curuser']->id == $details->owner || $moderate) ? anchor('torrent/edit/' . $details->id, "<span class='glyphicon glyphicon-pencil'></span>", array(
                'title' => 'Изминить',
                'style' => 'color:blue;'
            )) : "") ,
            'delete' => ($moderate ? anchor('torrent/delete/' . $details->id, "<span class='glyphicon glyphicon-trash'></span>", array(
                'title' => 'Удалить'
            )) : "") ,
            'related' => $this
                ->load
                ->view('templates/' . $this
                ->config
                ->item('default_theme') . '/helpers/related', array(
                'rows' => $this
                    ->details_model
                    ->get_related($title, $id, $details->category)
            ) , true) ,
        ...
        $this
            ->template->title = $details->name . ' | ' . $details->size;
        $this
            ->template
            ->content
            ->view('details', $data);
    ...
    public function edit($id)
    {
        ...
        if (!$this->data['logged_in']) show_404();
        ...
        if ($owner == false && !$this->data['admin_mod']) show_404();
        ...
        $this
            ->form_validation
            ->set_rules('title', 'Название', 'trim|required|min_length[5]|max_length[255]');
        $this
            ->form_validation
            ->set_rules('poster', 'Постер', 'trim|required|prep_url');
        $this
            ->form_validation
            ->set_rules('descr', 'Описание', 'trim|required|min_length[50]');
        $this
            ->form_validation
            ->set_rules('category', 'Категория', 'required|is_natural_no_zero');
            ...
            ->run() == false)
        {
            ...
            foreach ($this->data['categories'] as $category)
            {
                ...
                if ($category->id == $details->category) $cat .= " selected=\"selected\"";
        ...
        else
        {
            ...
            $this
                ->details_model
                ->update_info($id, $data); ///inserting data
            $this
                ->session
                ->set_flashdata('info', 'Торрент был успешно отредактирован!');
            CACHE()
                ->delete(md5('details_' . $id));
    ...
    public function add()
    {
        ...
        if (!$this->data['logged_in'])
        {
            $this
                ->session
                ->set_flashdata('info', 'Доступ к данной странице только для зарегистрированных пользователей!');
            redirect('auth/login', 'refresh');
        }

        $this
            ->breadcrumb
            ->append('Добавить новый торрент');
        ...
        $user = $this->data['curuser'];
        $new_filename = (string)'';
        if ($user->can_upload == 'no') show_error('Вам запрещено добавлять торренты!');
        ...
        $this
            ->form_validation
            ->set_rules('poster', 'Постер', 'trim|required|prep_url|valid_img');
        $this
            ->form_validation
            ->set_rules('descr', 'Описание', 'trim|required|min_length[50]');
        $this
        ...
        $config['upload_path'] = $this
            ->config
            ->item('public_folder') . 'upload/torrents/';
            ...
            ->do_upload('file'))
        {
            ...
            $save = ($this
                ->config
                ->item('save_torrent') == false ? 'no' : 'yes');
            $torrent = new Torrenting($torrent_file);
            $magnet = $torrent->magnet(); //generate magnet link
            $torrent->comment($this
                ->config
                ->item('site_name')); //set comment (sitename)
            $torrent->save($torrent_file);

            $info = $dict['info'];

            list($dname, $pieces) = array(
                $info['name'],
                $info['pieces']
            );

            if (isset($info['length'])) $totallen = $info['length'];
            ...
            else
            {
                ...
                foreach ($flist as $fn)
                {
                    ...
                    if (!count($ffa)) $torrent_errors .= "<p>filename error</p>";
                    $ffe = implode("/", $ffa);
                    $filelist[] = array(
                        $ffe,
                        $ll
            ...
            $q = $this
                ...
                ->select("id, name, url")
                ->from("torrents")
                ->where("info_hash", (function_exists('hex2bin') ? hex2bin($infohash) : pack('H*', $infohash)))->get()
                ->row();

            if ($q) $torrent_errors .= "<p>Данний релиз уже есть на сайте: " . anchor('torrent/' . $q->id . '-' . $q->url, $q->name) . "</p>";
        }
        else
        {
            $torrent_errors = $this
		...
		'required' => 'required'
            ...
            $this
                ->template->title = 'Добавить торрент';
            $this
                ->template
                ->content
                ->view('add', $data);
        ...
        { ///IF NO ERRORS THAN WORK WITH DB
	    ...
	    if (!empty($dict['announce-list']))
            {
                ...
                foreach ($dict['announce-list'] as $al_url)
                {
                    ...
                    if ($al_url[0] == 'http://retracker.local/announce') continue;
                    if (!preg_match('#^(udp|http)://#si', $al_url[0])) continue;
                    if (in_array($al_url[0], $parsed_urls)) continue;
                    $url_array = parse_url($al_url[0]);
                    if (substr($url_array['host'], -6) == '.local') continue;
            ...
            if ($save == 'no' && file_exists($torrent_file)) unlink($torrent_file);
    ...
    public function delete($id)
    {
        ...
        $this
            ->details_model
            ->delete_torrent($id); ///inserting data
        if (file_exists($this
            ->config
            ->item('public_folder') . 'upload/torrents/' . $details->file_name . '.torrent')) unlink($this
            ->config
            ->item('public_folder') . 'upload/torrents/' . $details->file_name . '.torrent'); //deleting the file
        $this
            ->session
            ->set_flashdata('info', 'Торрент был успешно удалён!');
    ...
    public function files($id)
    {
        ...
        if (!$this
            ->input
            ->is_ajax_request()) die('Not ajax request');

        $files = $this
            ->details_model
            ->get_files($id);

        if (!$files) die("Нет такого торрента!");

        $this
            ->load
            ->view('templates/' . $this
            ->config
            ->item('default_theme') . '/helpers/filelist', array(
            'files' => $files
        ));
    }
    ...
    public function trackers($id)
    {
        ...
        if (!$trackers) die("Нет такого торрента!");

        $this
            ->load
            ->view('templates/' . $this
            ->config
            ->item('default_theme') . '/helpers/trackerlist', array(
            'trackers' => $trackers
        ));
    }
    ...
    public function download($id)
    {
        ...
        if (file_exists($torrent_file) && $details)
        {
            ...
            if ((int)$details->category === $this
                ->config
                ->item('adult_cat'))
            {
                $torrent->announce($this
                    ->config
                    ->item('announce_urls') ['adult']);
                $torrent->comment(base_url() . 'torrent/' . $id);
                $torrent->save($this
                    ->config
                    ->item('public_folder') . '/temp/' . $name);
            }
            else
            {
                $torrent->announce($this
                    ->config
                    ->item('announce_urls') ['open']);
                $torrent->comment(base_url() . 'torrent/' . $id);
                $torrent->save($this
                    ->config
                    ->item('public_folder') . '/temp/' . $name);
            ...
            $data = file_get_contents($this
                ->config
                ->item('public_folder') . '/temp/' . $name); // Read the file's contents
            unlink($this
                ->config
                ->item('public_folder') . '/temp/' . $name);
            $this
                ->db
                ->set('downloaded', 'downloaded + 1', false)
                ->where('id', $id)->update('torrents');
    ...
    public function bookmarks($id = NULL)
    {
        if ($this->data['curuser'] !== false)
        {
                ...
                ->is_ajax_request())
            {
                ...
                $data = $this
                    ->details_model
                    ->bookmarks($id, $this->data['curuser']
                    ->id);
            ...
            else if ($id !== NULL)
            {
                ...
                $data = $this
                    ...
                    ->bookmarks($id, $this->data['curuser']
                    ->id);
                ($data['class'] == 'book') ? $this
                    ->session
                    ->set_flashdata('info', 'Закладка на раздачу успешно добавлена') : $this
                    ->session
                    ->set_flashdata('info', 'Закладка на раздачу успешно удалена');
                redirect('torrent/' . $id, 'refresh');
            }
            else if ($id === NULL and !$this
                ->input
                ->is_ajax_request())
            {
