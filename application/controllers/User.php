...
class User extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->breadcrumb->append('Личный кабинет', 'user/profile');

        $this->load->model('user_model');
	$this->load->library('Datatables');
        $this->template->content->view('helpers/user_tabs');
    }
    ...
    public function index($id = 0) {
        ...
        $user->country = $this->user_model->get_user_country($id)->name;
        $user->torrents = $this->user_model->count_user_torrents($id);
        $user->comments = $this->user_model->count_user_comments($id);


        $this->data['user'] = $user;
        $this->data['group'] = $this->ion_auth->get_users_groups($id)->row();
    ...
    function profile() {
        ...
        $this->breadcrumb->append('Редактировать профиль');
	...
	if(!file_exists($this->config->item('avatar_dir') . $user->userfile))
	{
        ...
        $config['max_width'] = $this->config->item('avatar_resize_w');
        $config['max_height'] = $this->config->item('avatar_resize_h');
        ...
        if (!empty($_FILES['userfile']['name']) && $this->upload->do_upload('userfile')) {
            ...
            $file = $this->upload->data(); //store file info
        ...
        if ($this->form_validation->run() == FALSE || $image_errors != '') {
            ...
            $this->session->set_flashdata('info', "Профиль обновлён");
    ...
    function torrents() {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $this->datatables->select('id, name, size, added, seeders, leechers, url, owner')
                    ...
                    ->from('torrents')
                    ->where(array('owner' => $this->data['curuser']->id));

            echo $this->datatables->generate();
        } else {
            $this->breadcrumb->append('Мои торренты');
            $data = array(
                'torrenttable' => view_helper('ttable', '', true)
            );

            $this->template->title = 'Мои торренты';
            $this->template->content->view('browse', $data);
            $this->template->publish();
        }
    }
    ...
    function bookmarks() {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $this->datatables->select('b.t_id, t.id as id, t.name as name, t.size as size, t.added as added, t.seeders as seeders, t.leechers as leechers, t.url as url, t.owner as owner')
                    ->edit_column('name', ($this->data['logged_in'] && $this->data['admin_mod'] ? anchor('torrent/edit/$2', "<span class='glyphicon glyphicon-pencil'></span>", array('title' => 'Изминить', 'class' => 'torrenttable_edit')) : "") . anchor('torrent/$2-$3', '$1'), 'name, id, url, owner')
                    ->from('bookmarks b')
                    ->where(array('b.user_id' => $this->data['curuser']->id))
                    ->join('torrents t', 't.id = b.t_id', 'left');
//                    ->edit_column('name', ($this->data['logged_in'] && $this->data['admin_mod'] ? anchor('torrent/edit/$2', "<span class='glyphicon glyphicon-pencil'></span>", array('title' => 'Изминить', 'class' => 'torrenttable_edit')) : "") . anchor('torrent/$2-$3', '$1'), 'name, id, url, owner');

            echo $this->datatables->generate();
        } else {
            $this->breadcrumb->append('Мои закладки');
            $data = array(
                'torrenttable' => view_helper('ttable', '', true)
            );

            $this->template->title = 'Мои закладки';
            $this->template->content->view('bookmarks', $data);
            $this->template->publish();
        }
    }
    ...
    function comments() {
        ...
        $this->breadcrumb->append('Мои комментарии');
        ...
        $comments = $this->user_model->comments($user->id, $limit, $offset);
        ...
        $this->template->title = 'Мои комментарии';
        $this->template->content->view('user_comments', $data);
        $this->template->publish();
    }
