...
class Tracker extends CI_Controller
{
  function __construct() {
    parent::__construct();
		...
		{
			$this->$var_name = (float) $this->input->get($var_name) ?? NULL;
		}
    ...
    if (!$this->input->valid_ip($this->ip))
		{
		...
		// Start announcer
		//
		$info_hash_sql = rtrim($this->db->escape_str($this->info_hash), ' ');
		// Stopped event
		if ($this->event === 'stopped')
		{
			$this->db->delete('tracker', ['info_hash' => $info_hash_sql, 'ip' => $ip_sql, 'port' => $this->port]);
			die();
		}	
	  $data = [
	  		...
	  		'ip' => $ip_sql,
	  		'port' => $this->port,
	  		'update_time' => $this->timenow
	  	];
	  $this->db->replace('tracker', $data);
...
//		$info_hash_sql = $this->db->escape($this->info_hash);
		...
		{
			$row = $this->db->select('completed,seeders,leechers')
								->where(['info_hash' => $info_hash_sql])
								->limit('1')
								->get('torrents')->row_array();
		  //var_dump($rowset);
			$output = [
			  'complete'   => (int)$row['seeders']+rand(10, 90),
			  'downloaded' => (int)$row['completed']+rand(10, 90),
			  'incomplete' => (int)$row['leechers']+rand(10, 90),
			  'info_hash'  => $this->info_hash
			];
			CACHE()->save('scrape_'.md5($this->info_hash), $output, CACHE_LIFE_TIME());
		}
	//var_dump($output);
	// Return data to client
	echo $this->_bencode($output);
