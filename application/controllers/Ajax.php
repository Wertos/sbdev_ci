...
class Ajax extends MY_Controller {
  ...
  function __construct() {
      ...
      $ajax = new stdClass();
      $ajax->action = $this->uri->segment(2);
  ...
  public function delann() {
	  $tid = (int) $this->input->post('tid') ?? 0;
	  $id  = (int) $this->input->post('id') ?? 0;
	  $url = $this->input->post('url') ?? '';
	  $response = [];
#      die($tid.$url.$id);
	  $this->db->delete('torrents_scrape', ['id' => $id]);
  ...
  public function ajaxpag() {
	  $cat_id = (int) $this->input->post('catid') ?? 0;
	  $start = (int) $this->input->post('start') ?? 0;
	  $response = [];
	  $html = '';
	  if($start < 0) $start = 0;
	  $data = $this->browse->get_all_home($cat_id, $start);
	  ...
	  foreach($data as $row) {
			$metadata = '
				<ul class="list-inline home_metadata">
         		...
         		$html .= anchor('torrent/' . $row->id . ($row->url ? '-' . $row->url : ''),'<div class="poster-home" style="float:left;margin-right:3px;"><img data-html="true" title="<img width=\'100\' src=\''.$row->poster.'\' />" src="'.$row->poster.'" width="30" height="40" alt="" /></div><h5 class="list-group-item-heading" title="'.$row->name.'">' . character_limiter($row->name, 500) . '</h5>' . $metadata, array('class' => 'list-group-item', 'id' => 'torrent-'.$row->id));
	  }
	  $response['html'] = $html;
	  $response['stop'] = (count($data) < $this->config->item('home_per_cat') && $start != 0) ? 'true' : 'false';
