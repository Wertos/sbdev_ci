...
class Browse extends MY_Controller {
    ...
    public function home() {
        $this->breadcrumb->append($this->config->item('site_name') . ' - ' . $this->config->item('site_descr'));
    ...
    public function index() {
        ...
        if ($this->input->is_ajax_request()) {
            $this->datatables->select('id, name, size, added, seeders, leechers, url, owner, poster')
            ...
            $this->breadcrumb->append('Все раздачи');
            $data['torrenttable'] = view_helper('ttable', '', true);
            $this->template->title = 'Все раздачи';
    ...
    function search($str = '', $cat_id = 0) {
        ...
        if ($this->input->post('q'))
            redirect('browse/search/' . title_url($this->input->post('q')).$url_add);
        ...
        $cat_id = (int) $this->uri->uri_to_assoc(2)['category'];
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $this->breadcrumb->append('Результат поиска');
            $data['torrenttable'] = view_helper('ttable', '', true);
    ...
    public function category($url) {
        ...
        if ($this->input->is_ajax_request()) {
            $this->datatables
                    ...
                    ->edit_column('name', ($this->data['logged_in'] && $this->data['admin_mod'] ? anchor('torrent/edit/$2', "<span class='glyphicon glyphicon-pencil'></span>", array('title' => 'Изминить', 'class' => 'torrenttable_edit')) : "") . anchor('torrent/$2-$3', '$1'), 'name, id, url, owner')
                    ->from('torrents')
                    ->where(array('category' => $cid->id, 'modded' => 'yes'));
            echo $this->datatables->generate();
        } else {
            $this->breadcrumb->append('Все раздачи', 'browse');
            $this->breadcrumb->append($cid->name);
            $data['torrenttable'] = view_helper('ttable', '', true);
            $this->template->title = $cid->name;
            $this->template->content->view('browse', $data);
            $this->template->publish();
        }
    }
    ...
    public function user($id) {
        ...
        if ($this->input->is_ajax_request()) {
            $this->datatables
                    ...
                    ->edit_column('name', ($this->data['logged_in'] && $this->data['admin_mod'] ? anchor('torrent/edit/$2', "<span class='glyphicon glyphicon-pencil'></span>", array('title' => 'Изминить', 'class' => 'torrenttable_edit')) : "") . anchor('torrent/$2-$3', '$1'), 'name, id, url, owner')
                    ->from('torrents')
                    ->where(array('owner' => $id, 'modded' => 'yes'));
		echo $this->datatables->generate();
        } else {
            $this->breadcrumb->append('Все раздачи', 'browse');
            $this->breadcrumb->append($q->username);
            $data['torrenttable'] = view_helper('ttable', '', true);
            $this->template->title = $q->username;
            $this->template->content->view('browse', $data);
            $this->template->publish();
        }
    }
    ...
    public function sitemap() {
        $data = $this->browse->sitemap(); //select urls from DB to Array
        header("Content-Type: text/xml;charset=iso-8859-1");
        view_helper('sitemap', array('data' => $data));
    }

	public function rss($id = NULL) {
		if(!$logged_in) {
      	  $this->session->set_flashdata('info', 'Только для зарегистрированных !!');
      	  redirect('/', 'refresh');
		}
   		$data = array('feeds' => $this->browse->feeds_info($id));
   		header("Content-Type: application/rss+xml;charset=utf-8");
	    $this->load->view('templates/' . $this->config->item('default_theme') . '/rss_view', $data);
	}
